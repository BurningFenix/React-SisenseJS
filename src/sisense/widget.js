...
import { Col, Panel, ButtonGroup, DropdownButton, MenuItem } from 'react-bootstrap';
...
function exportCSV(widget){
  ...
  var elasticube = (typeof widget.$$model.datasource === "string") ? widget.$$model.datasource : widget.$$model.datasource.title;
  ...
  $http.post(url,JSON.stringify(jaql)).then(function(response){
      ...
      var isValid = (response.status === 200) && (response.data.headers) && (response.data.values);
      if (isValid){
          ...
          for (var i=0; i<response.data.values.length; i++){
              ...
              for (var j=0; j<response.data.values[i].length; j++){
                  thisRow += j<response.data.values[i].length ? response.data.values[i][j].data + "," : response.data.values[i][j].data;
              }
...
function exportImage(widget){
  ...
  var url = sisenseSettings.server + '/api/v1/dashboards/' + widget.$$dashboard.id
            + '/widgets/' + widget.id 
            + '/export/png?width=' + widgetContainer.offsetWidth
        ...
        <Panel /*bsStyle="primary"*/>
          <Panel.Heading>
            ...
            <ButtonGroup className="pull-right" >
              <DropdownButton title="" /*bsSize="xsmall" */id={'menu-' + this.state.widget.id}>
                <MenuItem eventKey="csv" onSelect={this.exportWidget}>Download CSV</MenuItem>
                <MenuItem eventKey="image" onSelect={this.exportWidget}>Download Image</MenuItem>
              </DropdownButton>
            </ButtonGroup>
          </Panel.Heading>
          <Panel.Body>
